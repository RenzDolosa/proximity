...
async function searchEmployees(query) {
  try {
    ...
    if (isQRCode) {
      ...
      url = `qr_tester_backend.php?${params.toString()}`;
      method = "GET";
    }
...
function renderResults(results, query) {
  ...
  if (!filteredResults || filteredResults.length === 0) {
    ...
    const filterText =
      currentFilter !== "all" ? ` (${currentFilter} filter applied)` : "";
    message.innerHTML = `
        <p class="no-results-message" id="no-results-message">
            No results found. 🔍
        </p>
  ...
  resultsBody.innerHTML = filteredResults
    .map((employee) => {
      ...
      // Check if QR code matches search query for highlighting
      const isQRMatch = qrCode.toLowerCase().includes(query.toLowerCase());
            ...
            <div class="${violation ? "div-with-violation" : "div-container"}">
                <div class="div-position">
                    ...
                    <p>${position}</p>
                        ${
                          image
                            ? `<img src="uploads/user_${currentUserId}/${image}" alt="${fullname}" class="employee-image" loading="lazy">`
...
function getCurrentUserId() {
  try {
    ...
    xhr.open("GET", "get_user_id.php", false); // synchronous
    xhr.send();
    if (xhr.status === 200) {
