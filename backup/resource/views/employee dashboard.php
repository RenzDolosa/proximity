  ...
  <main class="db-cont">
    ...
    <section class="stats-grid">
      <div class="stat-card">
        ...
        <img src="../assets/logo/mysql-logo.svg" loading="lazy" style="top: 10%; right: 2%; height: 40px; position:absolute;">
      </div>
      <div class="stat-card">
    ...
    <section class="stats-grid">
      <div class="stat-card">
        ...
        <img src="../assets/logo/database.svg" loading="lazy" style="top: 10%; right: 5%; height: 40px; position:absolute;">
      </div>
      <div class="stat-card">
        <div class="stat-icon" style="background: linear-gradient(135deg, #28a745, #20c997);">
          <i class="fas fa-user-check" style="z-index: 1000;"></i>
      ...
      <div class="stat-card">
        <div class="stat-icon" style="background: linear-gradient(135deg, #17a2b8, #6f42c1);">
          <img src="../assets/icon/nfc-icon.svg" loading="lazy" style="width: 32px; height: 32px; z-index: 1000; filter: invert(1);">
        </div>
      ...
      <div class="menu-card">
          ...
          <div class="activity-container">
            <?php foreach (array_slice($recentLogs, 0, 5) as $log): ?>
              <div class="activity-item">
                ...
                $userImage = $log['profile_image'] ?? $log['image'] ?? '../assets/icon/database-icon.png';
                $imagePath = "../../public/uploads/user/" . htmlspecialchars($userImage);

                // Check if image file exists, otherwise use default
                if (!file_exists($imagePath)) {
                  $imagePath = "../assets/icon/database-icon.png"; // Fallback icon
                }
                ?>
                <img src=<?php echo $imagePath; ?>
                  alt="<?php echo htmlspecialchars($log['fullname'] ?? 'User'); ?> Profile"
                  class="activity-icon"
                  onerror="this.src='../assets/logo/3PL.svg'; this.nextSibling.style.display='inline';">
        ...
        function getUserImage($log, $userId)
        {
          ...
          $imageSources = [
            ...
            $log['image'] ?? null,
            $log['avatar'] ?? null,
            '../assets/icon/database-icon.png', // Fallback icon
          ];

          foreach ($imageSources as $imageFile) {
            if ($imageFile) {
              $fullPath = "../../public/uploads/user/" . $imageFile;
          ...
          return "../assets/logo/3PL.svg";
        }
        ?>
        <hr style="border: none; height: 1px; background: #e1e5e9;">
        <?php if ($access['datalog']): ?>
          <div style="text-align: center; margin-top: 20px;">
            <a href="../../app/services/datalog.php" class="btn btn-secondary">View All Logs</a>
          </div>
        <?php endif; ?>
      </div>
    </section>
